...
export class FireStorageService {
  ...
  fileStorage(photoUrl: string, uid: string) {
    this.fser.firestore.collection('users').doc(uid).update({ photoUrl }).then(data => {
      console.log(data);
    }).catch(error => {
      console.log(error);
    });
  ...
  /*
    uploadTopItemImage(event: FileList, itemId: string, top: Top, items) {
      ...
      const fileStoragePath = `tops/${top.author}/${top.$key}/${itemId}`
      ...
      this.progressSnapshot = this.ngFireUploadTask.snapshotChanges().pipe(
        ...
        finalize(() => {
          ...
          this.fileUploadedPath.subscribe(resp => {
            const i = items.findIndex((e) => e.$key === itemId);
            items[i].image = resp;
            this.isImgUploading = false;
            this.isImgUploaded = true;
          }, error => {
